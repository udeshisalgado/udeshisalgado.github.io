...
- Cross-sectional training across vaccines with monthly data, using an <span style="color: blue;">expanding window approach</span> 
...
bcg_table <- data.frame(
  Model = c("GBM", "LinearRegression", "AutoARIMA", "SeasonalNaive", "AutoETS", "Naive"),
  MASE = c(0.7888, 0.7916, 0.7965, 0.8562, 0.9062, 2.5068),
  RMSSE = c(0.7373, 0.7256, 0.6895, 0.7929, 0.8100, 1.9648)
)
...
penta_table <- data.frame(
  Model = c("GBM", "SeasonalNaive", "AutoARIMA", "AutoETS", "LinearRegression", "Naive"),
  MASE = c(0.6672, 1.2129, 1.5655, 1.8941, 3.3084, 3.6028),
  RMSSE = c(0.6010, 0.9579, 1.1200, 1.3516, 2.2992, 2.4735)
)
...
mr_table <- data.frame(
  ...
  MASE = c(0.6762, 0.7434, 0.7524, 1.1277, 2.0586, 2.8525),
  RMSSE = c(0.6309, 0.7430, 0.5111, 1.2048, 1.7867, 2.3040)
)
...
opv_table <- data.frame(
  ...
  MASE = c(0.7529, 0.8397, 0.9261, 1.3501, 1.5331, 2.6915),
  RMSSE = c(0.6783, 0.7119, 0.8413, 1.0471, 1.1992, 2.0478)
)
